    ...
    <div id="media_v4" class="media discover">
      <div class="column_wrapper">
        <div class="content_wrapper wrap">
          ...
          <div class="search">
          <form id="inner_search_form" action="{{route('movies.search')}}" method="post">
              @csrf
              <label>
                <input dir="auto" id="inner_search_v4" name="query_str" type="text" tabindex="1" autocorrect="off" autofill="off" autocomplete="off" spellcheck="false" placeholder="Buscar una película......" value="">
              </label>
          ...
          <div class="column_content flex scroller">
            @if(count($movies) > 0)
              ...
              <?php $movie = (object) $movie ?>
                @if(!empty($movie->id) && !empty($movie->title) && !empty($movie->poster_path) && !empty($movie->vote_average))
